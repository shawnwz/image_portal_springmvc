...
public class ImageGalleryController extends BaseController {
	...
	@RequestMapping(value = { "/imggallery{sid}", "/imggallery/{sid}",
			"/imggallery/{sid}" })
	public String imagessets(HttpServletRequest request,HttpServletResponse response, ModelMap modelMap,
		...
		if(s.equals("1"))
		{
			imageGalleryService.cleanCache();
		}
		...
		if (sid == null || sid <= 0)
		{
			...
			modelMap.addAttribute("item1", variablesService.getPpsEntityValue("flow_imageGalleryLanding_1"));
			modelMap.addAttribute("list",imageGalleryService.getTopList(p,-1));
			modelMap.addAttribute("map",imageGalleryService.getTopMap(p,-1));
			int total=imageGalleryService.getTopCount();
			int tp=total/Constants.imageSet_PageSize+(total%Constants.imageSet_PageSize==0?0:1);
			modelMap.addAttribute("tp",tp);
			modelMap.addAttribute("p",p);
			return "page/imagegallerylanding";
		}else {
			modelMap.addAttribute("item1", imageGalleryService.getItem(sid));
			modelMap.addAttribute("list",imageGalleryService.getTopList(1,1000));
			modelMap.addAttribute("map",imageGalleryService.getTopMap(1,1000));
			List<PreviewImageInfo> list=imageGalleryService.getImages(sid);
			modelMap.addAttribute("imglist",list);
			modelMap.addAttribute("imgcount",list.size());
			return "page/imagesgallery";
		}
	...
	@RequestMapping(value = {"/imagegalleryview/{sid}/{gaopinId}.jpg"})
	@ResponseBody
	public void showPreview(HttpServletRequest request, HttpServletResponse response, @PathVariable String gaopinId,@PathVariable int sid){
		...
		if(!imageGalleryService.checkImages(sid, gaopinId)) return;
		...
		try{
			if(!showFile.exists()){
				...
				showFile=new File(ImageGalleryController.class.getResource("/default.jpg").getFile());
				if(!showFile.exists()){
					return;
				}
			}
