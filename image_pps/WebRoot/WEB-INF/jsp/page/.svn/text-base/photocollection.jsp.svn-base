...
<div class="main clearfix padding-lr35">
    ...
    <div class="photo_div_top">
        <div class="photo_tt">
        <h1>图片库：${name}</h1>
        <h2 class="mtp5"><a href="/search#p=1&cid=${curitem.id}" class="c6699cc">查看所有 ${name} 的图片</a></h2>
        </div>
    ...
    <div class="mtp10 browse-filmstrip">
    	<div class="header">
        	<h1>${name}的最近的图片</h1>
            <p class="mtp10">这是我们的 ${name} 图库最近新增加的图片。每次添加了新图片，此页面就会自动更新。点击以下缩略图便可查看该图片的更多详情。</p>
        </div>
		<div class="filmstrip-navigation">
			...
			<a href="/search#p=1&cid=${curitem.id}">查看所有图片</a>		</div>
            ...
            <div class="slides_container" style="width:auto;" id="scrollCont_02">
            	                <div class="slides" style="width:auto;">
                	                  <c:forEach items="${map}" var="item" varStatus="stat">
								 		...
								 		<dd><a href="/photographer/${item.photographerName}" class="c6699cc">${item.photographerName}</a></dd><dd>${item.title}</dd></dl>

    									</c:forEach>
    						 </div>
                            </div>
            
        	<span class="ckz mrt14"><a href="search#p=1&cid=${curitem.id}">查看所有图片</a></span>
        </div>
