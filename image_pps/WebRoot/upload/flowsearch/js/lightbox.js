...
function addToLightBox(_imgId){
	...
	$.post(ajaxUrl_addToLightBox, {key : curLightBoxid,corbisId:_imgId}, function( data ) {
		if(data.error == 0)
		{
			...
			// 添加 此id 的图片数据 到灯箱尾端
			$('.jcarousel ul').append('<li><a href="image/'+data.data.corbisId+'"><img src="'+ data.map.imageHost+ 'THUMBNAIL/180/'+ data.map.path+ '" width="80" imgId="'+data.data.corbisId+'"/></a><span class="icon-font icon-8-delete"></span></li>');
			$('.jcarousel').jcarousel('reload');// 刷新灯箱
