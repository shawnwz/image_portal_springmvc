...
//***********************************************
jQuery.fn.wowSliderStack = function(D) {
	var L = jQuery;
	var l = this;
	var i = l.get(0);
	D = L.extend({
...
function ws_stack_vertical(d, a, b) {
	...
	this.go = function(k, h, n, m) {
		...
		c.each(function(o) {
			if (g && o != h) {
				this.style.zIndex = (Math.max(0, parseInt(this.style.zIndex)?parseInt(this.style.zIndex):1 + 1))//-1 to +1 jayzhu
			}
		});
		var j = e("ul", b);
		var l = document.all ? 0 : "0%";
		var orgiul=e("li", b).eq(0);//e(c.get(g ? k : h));
		var f = orgiul.clone().css({
			position : "absolute",
			"z-index" : 4,
			width : "100%",
			top : 0,
