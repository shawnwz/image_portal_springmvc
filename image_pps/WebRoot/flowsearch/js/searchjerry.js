...
//						myCurLightBoxs[n.id]=n;
					...
					{
						_curLightBoxid = data.data[data.data.length - 1].id; 
...
$('.resultPanelInner .searchKeyWord').on('click','.icon-8-delete',function(){
	...
	}else
	{
		...
		}else if( $('#'+checkId).hasClass('selectOne') )
		{
			...
			calcCheckBox($('#'+checkId).attr("name"));
			doSearch();
		}else if( checkId.indexOf("collection")==0) 
		{
		    $(this).parent().hide();
		    
		    if(advArr["cid"].length==1)
		    {
		    	delete advArr["cid"];
		    } else
		    { 
		        var newCidArr = advArr["cid"].toString().split(",");
		        var removedCid = checkId.substr(checkId.length-3,3);
		    
		        for(var i=0; i<newCidArr.length; i++)
		        {
		            if (newCidArr[i]==removedCid)
		            {
		        	    newCidArr.splice(i,1);	
		            }
		   	    }
		        advArr["cid"]= newCidArr;
		    }
		    doSearch();
		}
	...
	$('.galleryWrap .galleryThumb .imgWrap').tooltipster({
		...
		functionBefore: function(origin, continueTooltip) {
			...
			if (origin.data('ajax') !== 'cached') {
				...
				if(item)
				{
						htmlData='<div class="picDetailWrap '+imgLightBoxedClass+'"><div class="imgWrap"><img src="'+item.imageHost+'HOVER/'+item.storageId+'/'+item.path+ item.ext +'" imgid="'+item.corbisId+'" /></div><div class="picCont"><div class="tag"><span>'+(item.licenseType == 2 ? "RM" : "RF")+'</span>';
						var scn ="";
						if(item.scn != null)
						{
							scn =item.scn;
						...
						htmlData+='<br/>图片库:  '+item.collectionName+' <br/></div>';
...
//						}
					if(item.priType==0)
					{
					    htmlData+='<div class="btnWrap iconWrap">';
					    htmlData+='<a href="javascript:void(0)"  onclick="DownloadImage(\''+item.corbisId+'\');" class="icon-font icon-22-download"></a>';
					    htmlData+='<a href="javascript:void(0)" class="icon-font icon-11-favourite"></a>';
					    htmlData+='<a href="javascript:void(0)"  onclick="FindSameImage(\''+item.corbisId+'\');" class="icon-font icon-29-search2 jsSearchSimlar"></a></div>';
					}else if(item.priType==3)
					{
						htmlData+='<div class="btnWrap iconWrap btns2">';
						htmlData+='<a href="javascript:void(0)" class="icon-font icon-11-favourite"></a>';
						htmlData+='<a href="javascript:void(0)" onclick="FindSameImage(\''+item.corbisId+'\');" class="icon-font icon-29-search2"></a></div>';
					} else 
					{
						htmlData+='<div class="btnWrap iconWrap">';
					    htmlData+='<a href="javascript:void(0)"  onclick="DownloadImageWithWaterMark(\''+item.corbisId+'\');" class="icon-font icon-22-download"></a>';
					    htmlData+='<a href="javascript:void(0)" class="icon-font icon-11-favourite"></a>';
					    htmlData+='<a href="javascript:void(0)"  onclick="FindSameImage(\''+item.corbisId+'\');" class="icon-font icon-29-search2 jsSearchSimlar"></a></div>';
					}
			...
			if (data.success) {
				if (window.innerWidth > 1200){
					if(data.lt){
                    	...
                    	$("#winning").animate({width:"100%"},1000);	
						$("#close_win").css("display","block");	
						$("#winning").animate({paddingBottom:"30px"});
                	} else {
...
function showImages()
{
	...
	for(var i=0;i<currentData.data.length;i++)
	{
		...
		var html='<div href="javascript:void(0);" class="galleryThumb" style="" imgid="'+item.corbisId+'">'
			...
			+'<a href="javascript:void(0);" onclick="goDetail($(this));" ><img src="'+item.imageHost+'THUMBNAIL/'+currentSize+'/'+item.path+ item.ext+'" imgid="'+item.corbisId+'" imgRealId="'+item.id+'"></a>';

			if(item.collectionId==761 && item.isInImageGroup==2 && isPullStyle !=2 ){
				var line = (isPullStyle==0)?"30px":"25px";
				html += '<div class="groupStyle" style="height:'+line+';width:'+line+';"><a href="javascript:void(0)" onclick="goGroupLanding('+item.groupId+');" onmouseover="hideicon('+item.groupId+')" onmouseout="showicon('+item.groupId+')"><img class="icon-group" src="images/search/icon_group'+isPullStyle+'.png" id="img-'+item.groupId+'"/><div id="hideCount" class="blackgroundsix ' + item.groupId +'" style="display:none; width:'+line+';height:'+line+';"><span class="groupAmount_six" style="line-height:'+line+';">'+item.groupAmount+'</span></div></a></div>';
			}
			...
			if(item.priType==0){
				...
				html += '<a href="javascript:void(0)"  onclick="DownloadImageSepc(\''+item.corbisId+'\');" class="icon-font icon-31-shoppingCart2"></a>'; // 旗帜
         		...
         		html += '<a href="javascript:void(0)"  onclick="DownloadImageWithWaterMark(\''+item.corbisId+'\');" class="icon-font icon-22-download"></a>'; // 水印小图
			}
		//html+='<a href="javascript:void(0)" class="icon-font icon-22-download"></a> '
			...
			if(item.isInImageGroup!=0){
				html+='<a href="javascript:void(0)" onclick="goGroupLanding('+item.groupId+');" class="icon-font icon-6-collection"></a>'				
			}
			html+=
...
function initCidLabel()
{
	$(".label-collection").hide();
	var allVars = getUrlArgs();
	if(allVars['cid']) 
	{ 
		var cidArr=allVars['cid'].split(',');
		for (var j=0; j<cidArr.length; j++){
			var label="label-collection"+cidArr[j];
			$('#'+label).show();
		}
	}
}
...
function setPageNum() {
	if (currentData) {
		...
		var totalformated=commafy(currentData.total);
...
//带水印图片下载 type=5
function DownloadImageWithWaterMark(elesid, base)
{
	var b="";
	if(base&&base.length>0)
		b=base;
	location.href = b+"dl/getImg.do?type=5&corbisId="+elesid;
}
...
function goGroupLanding(groupId,base)
{
	var b="";
	if(base&&base.length>0)
		b=base;
...
{
		if(groupId !=0){
		 	$(".right_hide").show("fast",function(){
	   			$(".icon_group_id").text("图片组:"+currentData.groupId); 
				$(".icon_group_name").text(currentData.groupName);
			    if(	currentData.groupIntro ){
					$(".icon_group_describe").html(currentData.groupIntro+"<br>");
				} else {
					$(".icon_group_describe").attr("style", "display:none;");
				}
