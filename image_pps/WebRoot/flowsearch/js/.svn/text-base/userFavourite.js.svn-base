...
//										+ '"><p>' + n.value + '</p></a>');
						var nweFavFolder=$('<div class="favFolderBox" lightBoxId="'+n.id+'"><a href="my/lightbox?lightboxId='+n.id+'" class="folder"><div class="favFolderName">'+n.name+'</div><div class="favFolderAmount">共<span>'+n.imageNum+'</span>张图片</div></a><span class="delete_btn icon-font icon-cross" data-target=".bs-example-modal-sm"></span></div>').prependTo($('.favouriteFolderList'));
						// 使当前新建的灯箱 支持拖拽
...
//						$(".selectBox-dropdown-menu.selectBox-options.selectBox-options-bottom").append(phoneHtml);
					...
					$(".viewport").append('<div class="modal fade bs-example-modal-sm" tabindex="-1" role="dialog" aria-labelledby="mySmallModalLabel" aria-hidden="true"><div class="modal-dialog "><div class="modal-content modal-sm"><div class="modal-header"><button type="button" class="close new_close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button><h4 class="modal-title">删除灯箱</h4></div><div class="modal-body"><p>您的删除操作不可逆，是否确认删除？</p></div><div class="modal-footer"><button type="button" class="btn btn-default"  id="no_delete" data-dismiss="modal">否</button><button type="button" class="btn btn-primary  btnClose" id="yes_delete">是</button></div></div></div></div>');
...
//				+ '			<a href="javascript:void(0)" class="icon-font icon-31-shoppingCart2"></a>'
				...
				if(eles.location != '')
				{
					html+= '		'+ eles.location +'<br>';
				}
				
				html+= '		'+ getRightDate(eles.photoDate) +'<br>'
		...
		$(".pageNumWrap").find("input").keyup(function(){
			if(event&&event.keyCode==13){
				var pageNum = $(".pageNumWrap").find("input").val();
				mylightBoxSetShow(pageNum);
			}
...
//			+'<a href="javascript:void(0)" class="searchLabel">蔬菜</a>'
	...
	//点击删除X
	$('.viewport').on('click','.delete_btn',function(){
		lightbox_to_be_removed=$(this).closest('.favFolderBox').attr('lightBoxId');
		$(".bs-example-modal-sm").modal('show');
	});
	
	//删除灯箱
	$('.viewport').on('click','.btnClose',function(){
		$(".bs-example-modal-sm").modal('hide');
		$(".pageFavourite").removeClass("modal-open");
		$(".pageFavourite").removeAttr("style");
		$("div.modal-backdrop").remove();

		// 记录删除按钮按下时 灯箱id，
		var lightBoxId=lightbox_to_be_removed;
		lightbox_to_be_removed = -1;
...
/*	<div class="favFolderBox active" lightBoxId="1">
	...
	function createNewFavouriteFolder2()
	{
		$.post('my/addlightbox', {name :"新建收藏夹"}, function(data) {
			...
			if(data.error == 0)
			{
				var nweFavFolder=$('<div class="favFolderBox" lightBoxId="'+data.data.id+'"><a href="my/lightbox?lightboxId='+data.data.id+'" class="folder"><div class="favFolderName">新建收藏夹</div><div class="favFolderAmount">共<span>0</span>张图片</div></a><span class="delete_btn icon-font icon-cross" data-target=".bs-example-modal-sm"></span></div>').prependTo($('.favouriteFolderList'));
