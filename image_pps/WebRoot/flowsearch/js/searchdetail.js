...
//							myCurLightBoxs[n.id]=n;
	...
	{
		if(!groupId){
			$(".groupShown").css("display","none");
			return;
		}else {
		$(".groupImages").find("ul").html("");
		var paras="?groupId="+groupId+"&p=1";
		var reqParas={n:300,key:"",sortType:4,ssid:""};
		$.ajax({
			type : "POST",
			...
			success : function(data) {
				if (data.success) {
					...
					html1=$(".sameAlbum").html();
					html1+="("+length+")张";
					$(".sameAlbum").html(html1);
					...
					for(var i =0;i< length;i++)
					{
						...
						html+='    <img src="'+ data.data[i].imageHost+ 'THUMBNAIL/240/'+ data.data[i].path + data.data[i].ext + '">';
						html+='	</a>';
					...
					$(".groupImages").find("ul").html(html);
					 //  类似图片 切换
				    myTab('#myTabLeft4',"#myTabRight4");
				}
			},
			failed : function() {
...
function seeAllinGroup(groupId)
{
	window.open("search#groupId="+groupId+"&p=1");	
}
