...
var zindexdic=new Array();
//透明背影弹出框，点其他方关闭.
function boxshowkeepfornote(boxid){
	boxshow(boxid);
	if(!zindexdic["boxshowkeepfornote"+boxid])
		zindexdic["boxshowkeepfornote"+boxid]=$("#"+boxid).zIndex();
	//var zindex=$("#"+boxid).zIndex();
	$("#"+boxid).css("z-index","20000");
	$(".fullbackboxfornote").css("z-index","19999");
	$(".fullbackboxfornote").show();
	$(".fullbackboxfornote").one("click",function(){
		ClearFullback(function(){
			boxhidden(boxid);
		});
		$("#"+boxid).parent().css("z-index",zindexdic["boxshowkeepfornote"+boxid]);	
	});	
}
...
function boxshow(boxid){
	...
	$("#"+boxid).show();
	if($("#"+boxid).offset().top<0)
		$("#"+boxid).css("top",0);
	if($("#"+boxid).offset().left<0)
		$("#"+boxid).css("left",0);
}
...
function boxshowkeep(boxid)
{
	...
	if(!zindexdic["boxshowkeep"+boxid])
		zindexdic["boxshowkeep"+boxid]=$("#"+boxid).parent().zIndex();
	//var zindex=$("#"+boxid).parent().zIndex();
	$("#"+boxid).show();
	$("#"+boxid).css("z-index","20000");
	...
	$(".fullbackbox").one("click",function(){
		...
		$("#"+boxid).parent().css("z-index",zindexdic["boxshowkeep"+boxid]);	
	});	
	//加取消事件及滚动条初始化. 
	if(boxid=="h-search")
		{
...
function showbigimg(imgid){
	if(typeof isShowBigImage=="undefined"||!isShowBigImage)
		return;
//	console.log("showbigimg("+imgid+")");
	if(itemeventlist[imgid])
	{
		clearTimeout(itemeventlist[imgid]);
//		console.log("clearTimeouthide("+imgid+")");
	}
	curbigimg=imgid;
	itemeventShowlist[imgid]=setTimeout("showbigimgprivate('"+imgid+"')",200);
}
...
function showbigimgprivateloaded(imgid){
	...
	var cw=$(window).width()+document.body.scrollLeft;//document.documentElement.scrollLeft//document.body.scrollLeft
...
//	console.log(showbigimgw+":"+showbigimgh);
	
//	console.log("itemmin.offset().left+item.width()>cw"+itemmin.offset().left+":"+item.width()+":"+cw);
	if(itemmin.offset().left+item.width()/2+itemmin.width()/2>cw)
...
{
//	console.log("hidebigimg("+imgid+")");
	if(itemeventShowlist[imgid])
		{
		clearTimeout(itemeventShowlist[imgid]);
//		console.log("clearTimeoutshow("+imgid+")");
		}
	
	itemeventlist[""+imgid+""]=setTimeout("$(\"#bigdiv"+imgid+"\").fadeOut(200);",300);
	curbigimg="";
}

var itemeventlist=new Array();
var itemeventShowlist=new Array();
	...
	tagbar.each(function(){
		...
		item.find("a").click(function(){
			if($(this).attr("value")!=item.find("input").eq(0).val())
			{
				...
				item.find("input").eq(1).val($(this).find("p").text());
...
///用于input .text 的自动清空。在document ready时调用一次。
///举例 BindAutoClear($("#Name"),"输入文本...");
var BindAutoClearDic = {};
function BindAutoClear(item, str) {
	
	var orgicolor=item.css("color");
    if (item.val() == "" || item.val() == str) {
        item.val(str); item.css("color", "#C3C3C3");
    }
    
    item.bind("focus", function () {
        //item.select(); 
        if (item.val() == str || item.val() == '') {
            item.val("");
            item.css("color", orgicolor);
        }
    });
    item.bind("blur", function () { 
    	...
    	{ item.css("color", orgicolor); } });
    item.bind("click", function () { item.focus(); });
    item.bind("focusout", function () {
    //    item.blur(); 
    });
    
    BindAutoClearDic[item.selector]=str;
}

function BindAutoClearGetValue(item) {
    if (item.val() == BindAutoClearDic[item.selector])
        return "";
    else
        return item.val();
} 
function BindAutoClearResetValue(item) {
        if (item.val() == "" || item.val() == BindAutoClearDic[item.selector]) {
            item.val(BindAutoClearDic[item.selector]); item.css("color", "#C3C3C3");
        }
}
