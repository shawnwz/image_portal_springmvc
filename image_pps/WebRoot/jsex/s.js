...
function setShow() {
	...
	if(currentData.simUrl){
		$("#simimage").append('<a href="image/'+currentData.simId+'" target="_blank"><img src="'+currentData.simUrl+'" style="max-width: 50px;max-height: 40px;"/></a>');
	}
	...
	for (i = 0; i < data.length; i++) {
		...
		str+= '<p><b>' + rf + '</b>';
		//小图标~~~开始
		if(spl=='' && eles.corbisId.substring(0,3)=='SPL'){
			str += '<span style="float: right;background-image: url(../images/iconTotal.png);background-repeat: no-repeat;">&nbsp; &nbsp; &nbsp;'+eles.splashNum+'</span>'
			//str += '<span style="float: right;">'+eles.splashNum+'张</span>';
			//<span style="float: right;">70张</span>
		}
		else{
			...
			str += '<i class="pic-button8" onclick="LightBox($(this));" title="添加到灯箱"></i>'; // 灯箱
			str += '<i class="pic-button5 on" onclick="FindSameImage(\''+eles.corbisId+'\');" title="更多相似图片"></i>'; // ~~
			
			//以下二个是二选一的。 
			if(eles.priType==0){
				str += '<i class="pic-button6 on" onclick="DownloadImage(\''+eles.corbisId+'\');" title="下载"></i>'; // 下载
			}else if(eles.priType==3){
				str += '<i class="pic-button1 on" onclick="DownloadImageSepc(\''+eles.corbisId+'\');" title="旗帜"></i>'; // 旗帜
			}
		}
		//小图标结束~
		str += '<br />'+eles.scn+'<br />' + eles.collectionName + '<br />'; // 图库
		str += eles.corbisId+'' ;
		if(eles.pricing&&eles.pricing!='')
		str +='<span style="float: right;">['+eles.pricing+']</span><br/>';
		str +='<br/>';
...
//		console.log("mouseenter.imagelink170"+"image"+$(this).closest(".picbox-picbox").attr("imageid"));
	...
	if(currentData&&currentData.groupId)
	{
		...
		refreshlightbox();
		//set value . 
		$(".groupDetailsContainer .setstitle").html(currentData.groupName);
		$(".groupDetailsContainer .setsid").html(currentData.groupId);
		$(".groupDetailsContainer .setsCount").html(currentData.total);
		$(".groupDetailsContainer .setsdate").html(currentData.createTime);
		$(".groupDetailsContainer .setslocation").html(currentData.location);
		$(".groupDetailsContainer input").attr("groupid",currentData.groupId);
		if(currentData.photographer)
			{
			$(".groupDetailsContainer .setsphotographor").html(currentData.photographer);
			$(".groupDetailsContainer .setsphotographorinfos").show();
			}
		else
			{
	...
	}else
	{
		$(".groupDetailsContainer").hide();
		$(".forleftfilter").show();
		tag_change('piclist','piclisttag','0','2');
	}
...
function showClearword() {
	...
	$(".searchimformationboxlistdiv")
			.append(
					'<div class="icheckbox_minimal-aero"><input name="checkany" value="" type="checkbox"/><ins class="iCheck-helper" ></ins></div><label>任何</label><br/>');
	var selectedCla = cla || "";
	for (i = 0; i < currentData.claList.length; i++) {
		...
		// ?"checked":"";
		$(".searchimformationboxlistdiv").append(
				'<div class="icheckbox_minimal-aero"><input name="" value="' + currentData.claList[i].id
						+ '" type="checkbox"/><ins class="iCheck-helper" ></ins></div><label>'
						+ currentData.claList[i].items + '</label><br/>');
	}
	var searchimformationboxlistmarginbuttom=-20*(currentData.claList.length-1)-180;
	$("#searchimformationboxlist").css("margin-bottom",searchimformationboxlistmarginbuttom);
			...
			.iCheck(
					{
						...
						increaseArea : '20%',
						fathertag:'#searchimformationboxlist'// optional
					},
					function(item) {
...
function hiddenkeynote() {
	$.cookie('nk', "1");
	boxhidden('searchimformationboxkeynote');
}
